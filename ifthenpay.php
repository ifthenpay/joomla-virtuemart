...
class plgVmPaymentIfthenpay extends vmPSPlugin
{
	...
	function plgVmConfirmedOrder($cart, $order)
	{
		...
		if ($urlGatewayToPay == '') {
			...
			$urlError = urldecode(self::getCancelUrl($order)) . '&action=error';
	...
	public function getGatewayPaymentUrl($order)
	{
		...
		if (!empty($orderDtl)) {
			...
			$params['email'] = filter_var($orderDtl->email, FILTER_SANITIZE_EMAIL);
			$specialChars = array("@", "!", "#", "$", "%", "^", "&", "|", "*", "(", ")", "-", "_", "=", "+", "{", "}", "[", "]", ":", ";", "<", ">", ",", ".", "?", "/");
			// $params['firstName'] = str_replace($specialChars, '', $orderDtl->first_name);
			$params['firstName'] = str_replace($specialChars, '', $orderDtl->first_name);
			$params['lastName'] = str_replace($specialChars, '', $orderDtl->last_name);
			$params['returnUrl'] = urldecode(self::getReturnUrl($order));
			$params['cancelUrl'] = urldecode(self::getCancelUrl($order)) . '&action=cancel';
			...
			// $params['btn_close_label'] = "VOLTAR";

			$params['errorUrl'] = urldecode(self::getCancelUrl($order)) . '&action=error';
	...
	function plgVmOnUserPaymentCancel()
	{
		...
		if (vRequest::getString('action', '') == 'error') {
	...
	{
		return JURI::root() . "index.php?option=com_virtuemart&view=pluginresponse&task=pluginresponsereceived&pm=" . $paymentMethodId .
			'&oid=' . $orderNumber;
	}

	static function getReturnUrl($order)
	{
		return JURI::root() . "index.php?option=com_virtuemart&view=pluginresponse&task=pluginresponsereceived&pm=" . $order['details']['BT']->virtuemart_paymentmethod_id .
